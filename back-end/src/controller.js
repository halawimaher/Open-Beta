  ...
  try {
    ...
    const getSkillsList = async () => {
      const rows = await db.all(
        "SELECT id, label, description, image FROM Skills"
      );
      return rows;
    };
    ...
    const getExperienceList = async () => {
      const rows = await db.all(
        "SELECT id, role, company, start, end, image FROM Experience"
      );
      return rows;
    };
    ...
    const getProjectsList = async () => {
      const rows = await db.all(
        "SELECT id, name, github_link, demo_link, description, image FROM Projects"
      );
      return rows;
    };
    ...
    const getContactLinks = async () => {
      const rows = await db.all(
        "SELECT id, facebook_link, youtube_link, twitter_link, email FROM Contact_Links"
      );
      return rows;
    };
    ...
    const getExperienceByID = async (id) => {
      const rows = await db.all(
        `SELECT id, role, company, start, end, image FROM experience WHERE id=${id}`
      );
      return rows;
    };
    ...
    const getProjectByID = async (id) => {
      const rows = await db.all(
        `SELECT id, name, github_link, demo_link, description, image  FROM projects WHERE id=${id}`
      );
      return rows;
    };
    ...
    const getAboutByID = async (id) => {
      const rows = await db.all(
        `SELECT id, title, about_text FROM about WHERE id=${id}`
      );
      return rows;
    };
    ...
    const getLinkByID = async (id) => {
      const rows = await db.all(
        `SELECT id, facebook_link, youtube_link, twitter_link, email FROM contact_links WHERE id=${id}`
      );
      return rows;
    };
    ...
    const createSkill = async (name, label, description, image) => {
      const rows = await db.run(
        `INSERT INTO Skills (name, label, description, image) VALUES ("${name}", "${label}", "${description}"), "${image}"`
      );
      return rows.lastID;
    };
    ...
    const createExperience = async (role, company, start, end, image) => {
      const rows = await db.run(
        `INSERT INTO Experience (role, company, start, end, image) VALUES ("${role}", "${company}", "${start}", "${end}", "${image}")`
      );
      return rows;
    };
    ...
    const createProject = async (
      ...
      const rows = await db.run(
        `INSERT INTO Projects (name, github_link, demo_link, description) VALUES ("${name}", "${github_link}", "${demo_link}", "${description}", "${image}")`
      );
      return rows;
    };
    ...
    const createAbout = async (title, about_text) => {
      const rows = await db.run(
        `INSERT INTO About (title, about_text) VALUES ("${title}", "${about_text}")`
      );
      return rows;
    };
    ...
    const createHome = async (title, description) => {
      const rows = await db.run(
        `INSERT INTO Home (title, description) VALUES ("${title}", "${description}")`
      );
      return rows;
    };
    ...
    const createLink = async (
      ...
      const rows = await db.run(
        `INSERT INTO Contact_Links (facebook_link, youtube_link, twitter_link, email) VALUES ("${facebook_link}", "${youtube_link}", "${twitter_link}", "${email}")`
      );
      return rows;
    };
    ...
    const updateExperience = async (id, role, company, start, end, image) => {
      ...
      SET role = '${role}', company='${company}', start = ${start}, end = '${end}', image='${image}' WHERE id=${id}`);
      return rows;
    };
